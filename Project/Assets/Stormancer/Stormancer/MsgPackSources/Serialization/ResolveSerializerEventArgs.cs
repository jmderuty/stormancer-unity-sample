...
namespace MsgPack.Serialization
{
	...
	public sealed class ResolveSerializerEventArgs : EventArgs
	{
        ...
        public void SetSerializer(Type type, MessagePackSerializer foundSerializer)
        {
            if(type != this.TargetType || !typeof(MessagePackSerializer<>).MakeGenericType(type).IsInstanceOfType(foundSerializer))
            {
                throw new InvalidOperationException(
                    String.Format(
                        CultureInfo.CurrentCulture,
